...
public class Laptop {
    ...
    double prixVente;
    public Laptop(int idLaptop, String model, Marque marque, Processeur processeur, Ram ram, double tailleEcran, Disque disque, double prixAchat, double prixVente)
    throws Exception {
    ...
    public TransfertLaptop getTransfertLaptopEncours(Connection con, PointVente pointVente)
    throws Exception {
        TransfertLaptop result=null;
        boolean jAiOuvert=false;
        if(con==null) {
            jAiOuvert=true;
            con=ConnexionBdd.connexionPostgress("postgres", "AnaTaf37", "mikolo");
        ...
        ResultSet resultSet=null;
        try {
            String sql="select*from v_transfert_laptop_en_cours where id_laptop=? and id_point_vente=?";
            preparedStatement=con.prepareStatement(sql);
            preparedStatement.setInt(1, this.getIdLaptop());
            preparedStatement.setInt(2, pointVente.getIdPointVente());
            resultSet=preparedStatement.executeQuery();
            while(resultSet.next()) {
                result=new TransfertLaptop(resultSet.getInt("id_transfert_laptop"), pointVente, this, resultSet.getInt("quantite_laptop"), resultSet.getDate("date_transfert"), resultSet.getInt("etat_transfert"));
            }
        } catch (Exception e) {
            throw e;
        } finally {
